...
void SceneReader::parseTokenBlock(std::queue<std::string> &tokenBlock) {
  ...
  if (blockType == "SCENE") {
    ...
    std::cerr << "Unexpected block type '" << blockType << "' starting on line " <<
              startLine_ << std::endl;
    exit(-1);
  }
}
...
void SceneReader::parseSceneBlock(std::queue<std::string> &tokenBlock) {
  while (tokenBlock.size() > 0) {
    ...
    if (token == "AMBIENTLIGHT") {
      ...
      std::cerr << "Unexpected token '" << token << "' in block starting on line " <<
                startLine_ << std::endl;
      exit(-1);
    }
  }
...
double SceneReader::parseNumber(std::queue<std::string> &tokenBlock) {
  ...
  if (endPtr != token.c_str() + token.length()) {
    std::cerr << "Expected a number but found '" << token <<
              "' in block starting on line " << startLine_ <<
              std::endl;
    exit(-1);
  }
...
void SceneReader::parseCameraBlock(std::queue<std::string> &tokenBlock) {
  ...
  if (cameraType == "PINHOLECAMERA") {
    ...
    std::cerr << "Unexpected camera type '" << cameraType <<
              "' in block starting on line " << startLine_ <<
              std::endl;
    exit(-1);
  }
  ...
  while (tokenBlock.size() > 0) {
    ...
    if (token == "ROTATE") {
      ...
      if (axis == "X") {
        ...
        std::cerr << "Unexpected axis '" << axis << "' in block starting on line " <<
                  startLine_ << std::endl;
        exit(-1);
      }
...
void SceneReader::parseLightBlock(std::queue<std::string> &tokenBlock) {
  ...
  if (lightType == "POINTLIGHT") {
    ...
    std::cerr << "Unexpected light type '" << lightType <<
              "' in block starting on line " << startLine_ <<
              std::endl;
    exit(-1);
  }
...
void SceneReader::parseObjectBlock(std::queue<std::string> &tokenBlock) {
  ...
  if (objectType == "SPHERE") {
    ...
    if (csgType != "UNION" && csgType != "INTERSECTION"
        && csgType != "DIFFERENCE") {
      std::cerr << "Unimplemented CSG operation '" << csgType <<
                "' in block starting on line " << startLine_ <<
                std::endl;
      exit(-1);
    }
    ...
    if (tokenBlock.front() != "OBJECT") {
      std::cerr << "Missing left CSG tree Object in block starting on line " <<
                startLine_ << std::endl;
      exit(-1);
    }
    ...
    if (tokenBlock.front() != "OBJECT") {
      std::cerr << "Missing right CSG tree Object in block starting on line " <<
                startLine_ << std::endl;
      exit(-1);
    }
    ...
    std::cerr << "Unexpected object type '" << objectType <<
              "' in block starting on line " << startLine_ <<
              std::endl;
    exit(-1);
  }
  ...
  while (tokenBlock.size() > 0) {
    ...
    if (token == "ROTATE") {
      ...
      if (material == materials_.end()) {
        std::cerr << "Undefined material '" << materialName <<
                  "' in block starting on line " << startLine_ <<
                  std::endl;
        exit(1);
      } else {

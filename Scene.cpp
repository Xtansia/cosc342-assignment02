...
Scene::Scene() : backgroundColour(0, 0, 0), ambientLight(0, 0, 0),
  maxRayDepth(3), renderWidth(800), renderHeight(600),
...
void Scene::render() const {
  ...
  std::cout << "Rendering a scene with " << objects_.size() << " objects" <<
            std::endl;
...
bool Scene::isInShadowFromLight(const Point &point,
                                const std::shared_ptr<LightSource> &light) const {
  Ray shadowRay;
...
Colour Scene::computeColour(const Ray &viewRay, unsigned int rayDepth) const {
  ...
  for (auto &light : lights_) {
    ...
    diffuseColour = mat.diffuseColour * std::max(unitLightDirection.dot(unitNormal),
                    0.0);

    // Calculate the direction of the reflected light, r = 2n(l.n)-l
    reflectedLightDirection = 2 * unitNormal * unitLightDirection.dot(
                                unitNormal) - unitLightDirection;
    unitReflectedLightDirection = reflectedLightDirection /
                                  reflectedLightDirection.norm();

    // Calculate specular colour (Phong) provided by this light source
    specularColour = mat.specularColour * std::pow(std::max(unitEyeDirection.dot(
                       unitReflectedLightDirection), 0.0), mat.specularExponent);

    // Add the diffuse and specular colours multiplied by the light source's illumination
    hitColour += light->getIntensityAt(point) * light->colour *
  ...
  if (rayDepth > 0 && mat.mirrorColour.red > epsilon
      && mat.mirrorColour.green > epsilon && mat.mirrorColour.blue > epsilon) {
    // Cast reflection ray, r = 2n(e.n)-e
    Ray reflectionRay;
    reflectionRay.point = point;
    reflectionRay.direction = 2 * unitNormal * unitEyeDirection.dot(
                                unitNormal) - unitEyeDirection;
